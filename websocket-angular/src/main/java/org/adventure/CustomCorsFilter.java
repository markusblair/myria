...
public final class CustomCorsFilter implements Filter {
    ...
    public void doFilter(final ServletRequest servletRequest,
        ...
        if (!(servletRequest instanceof HttpServletRequest) ||
            ...
            throw new ServletException("CustomCorsFilter.onlyHttp");
        }
        ...
        CustomCorsFilter.CORSRequestType requestType = checkRequestType(request);

        // Adds CORS specific attributes to request.
        if (decorateRequest) {
            CustomCorsFilter.decorateCORSProperties(request, requestType);
        }
        switch (requestType) {
        case SIMPLE:
	...
	protected void handleSimpleCORS(final HttpServletRequest request,
        ...
        CustomCorsFilter.CORSRequestType requestType = checkRequestType(request);
        if (!(requestType == CustomCorsFilter.CORSRequestType.SIMPLE ||
                requestType == CustomCorsFilter.CORSRequestType.ACTUAL)) {
            throw new IllegalArgumentException("CustomCorsFilter.wrongType2");
        }

        final String origin = request
                .getHeader(CustomCorsFilter.REQUEST_HEADER_ORIGIN);
        ...
        if (anyOriginAllowed && !supportsCredentials) {
            ...
            response.addHeader(
                    CustomCorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_ORIGIN,
                    "*");
        } else {
        ...
        if (supportsCredentials) {
            response.addHeader(
                    CustomCorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_CREDENTIALS,
                    "true");
        }
        ...
        if ((exposedHeaders != null) && (exposedHeaders.size() > 0)) {
            ...
            response.addHeader(
                    CustomCorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_EXPOSE_HEADERS,
    ...
    protected void handlePreflightCORS(final HttpServletRequest request,
        ...
        String accessControlRequestMethod = request.getHeader(
                CustomCorsFilter.REQUEST_HEADER_ACCESS_CONTROL_REQUEST_METHOD);
        ...
        String accessControlRequestHeadersHeader = request.getHeader(
                CustomCorsFilter.REQUEST_HEADER_ACCESS_CONTROL_REQUEST_HEADERS);
        ...
        if (preflightMaxAge > 0) {
            response.addHeader(
                    CustomCorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_MAX_AGE,
                    String.valueOf(preflightMaxAge));
        }

        // Section 6.2.9
        response.addHeader(
                CustomCorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_METHODS,
                accessControlRequestMethod);

        // Section 6.2.10
        if ((allowedHttpHeaders != null) && (!allowedHttpHeaders.isEmpty())) {
            response.addHeader(
                    CustomCorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_HEADERS,
    ...
    private void handleInvalidCORS(final HttpServletRequest request,
            final HttpServletResponse response, final FilterChain filterChain) {
        String origin = request.getHeader(CustomCorsFilter.REQUEST_HEADER_ORIGIN);
    ...
    protected static void decorateCORSProperties(
        ...
        if (corsRequestType == null) {
            throw new IllegalArgumentException("CustomCorsFilter.nullRequestType");
        }

        switch (corsRequestType) {
        case SIMPLE:
            request.setAttribute(
                    CustomCorsFilter.HTTP_REQUEST_ATTRIBUTE_IS_CORS_REQUEST,
                    Boolean.TRUE);
            request.setAttribute(CustomCorsFilter.HTTP_REQUEST_ATTRIBUTE_ORIGIN,
                    request.getHeader(CustomCorsFilter.REQUEST_HEADER_ORIGIN));
            request.setAttribute(
                    CustomCorsFilter.HTTP_REQUEST_ATTRIBUTE_REQUEST_TYPE,
        ...
        case PRE_FLIGHT:
            ...
            request.setAttribute(
                    CustomCorsFilter.HTTP_REQUEST_ATTRIBUTE_REQUEST_HEADERS, headers);
            break;
        case NOT_CORS:
            request.setAttribute(
                    CustomCorsFilter.HTTP_REQUEST_ATTRIBUTE_IS_CORS_REQUEST,
    ...
    private void parseAndStore(final String allowedOrigins,
        ...
        if (preflightMaxAge != null) {
            try {
                ...
                throw new ServletException("CustomCorsFilter.invalidPreflightMaxAge", e);
            }
        }
